...
const postController = {
    ...
    getById: async (req, res) => {
        try {
            const postId = req.params.id;
            const post = await db.one('SELECT p.id, p.title, p.description, p.created_at, COUNT(l.id) AS likes_count, json_agg(json_build_object(\'id\', c.id, \'comment\', c.comment, \'user_id\', c.user_id)) AS comments FROM posts p LEFT JOIN likes l ON p.id = l.post_id LEFT JOIN comments c ON p.id = c.post_id WHERE p.id = $1 GROUP BY p.id', postId);
            return res.json(post);
        } catch (error) {
            console.error(error);
            return res.status(500).json({ error: 'Internal server error' });
